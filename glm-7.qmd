...
# working with small estimates are no fun. 
...
glm.8b <-
  brm(family = negbinomial,
      y ~ 0 + Intercept + roach100 + treatment,
...
Findings across marginal are slightly different.
...
```{r}
predictions(glm.8b, newdata = datagrid(roach100 = c(0,.1,1,5), treatment = unique))
```


```{r}
predictions(glm.7, newdata = datagrid(roach1 = c(0,10,100,500), treatment = unique))
```
...
```{r}
#| code-fold: true
predictions(glm.8b,by = c("roach100", "treatment")) %>% 
  posterior_draws() %>%
  mutate(treatment = as.factor(treatment)) %>% 
  ggplot(aes(x = roach100, y = draw, color = treatment)) +
  stat_lineribbon(.width = .95, alpha = .4) + ylab("roaches")
```
