...
-   What do our priors say about what our model expects? This is a way to check if our priors are too liberal or conservative.
...
We sometimes want to get a slope or estimate at different levels of variables. From {marginalmeans} we can use the slopes function. Note that our slope should be the same at all levels of X.
...
- If not, then you should question both your model and the posterior. Aka a method for doing

- Will can use the model+posterior to generate (new) predictions about novel scenarios, e.g., those that don't exist in the dataset
