...
Mr = Multi-level regression aka MLM 

P = poststratification, with post for posterior and stratification for stratify.
...
```{r}
predictions(mlm.mrp,
    variables = "educ_group", 
    newdata = datagrid(state_name = "missouri"))
```
...
p4 <-
  pred %>%
  mutate(type = factor("mrp.margeff")) %>% 
  ggplot(aes(x = estimate, xmin = conf.low, xmax = conf.high, y = age_group)) + 
  geom_pointrange(color = "red", size = 0.8, fatten = 2) +
  scale_x_continuous(breaks = c(0, .5, 1), limits = c(0, 1)) +
  facet_wrap(~type)
...
#| code-fold: true
 predictions(mlm.mrp,
  ...
  sort_by(~ estimate) |>
  transform(state = factor(state_name, levels = state_name)) %>% 
ggplot(aes(y = state, xdist = rvar)) + 
...
This extends beyond Gaussian DVs, as most response distributions have a "location" parameter and one or more "scale" or "shape" parameters (remember negative binomial?). Instead of only predicting the location parameters, we can also predict the scale parameters
...
mlm %>% 
  ...
  add_epred_draws(d.1a) %>% 
  ggplot(aes(x = .epred, group =  fct_rev(group), fill = fct_rev(group))) +
...
melsm.2 %>% 
spread_draws(b_sigma_Intercept,r_record_id__sigma[ID, term]) %>% 
   median_qi(estimate = exp(b_sigma_Intercept + r_record_id__sigma)) %>% 
...
One you have the 10 datasets you run the model separate in each of them. 
...
Missingness is treated as a lack of information problem. But we provide create a model to help with prediction. So we will use our model to make predictions of missing parameters. 
...
parameter for each missing variable 
...
4000* * 10 (missing for chl) = 40,000 predictions 
...
ME.2a <- brm(schoool.success | mi(2.74) ~ me(happiness, .85), 
            data = MI, 
            iter = 2000, warmup = 1000, chains = 4, cores = 4,
            backend = "cmdstanr", 
            file = "ME.2a")
...
Map items onto a latent construct. Unlike standard CTT, we assume: 
  1) items are not interchangeable. In standard approaches more items, better reliability
  2) some items are better suited for different levels of the latent construct. E.g. a consideration of hard vs easy items  
  3) focus on test information over reliability
  
## IRT

- Attempts to simultaneously assess parameters of the item along with parameters of the person

- In this way it is similar to an multivariate/SEM model, a distributional model, and a MLM model. 
...
All regression models are linear in that they consist of a set of liner combinations. b3 X1 * X2 is totally fine as interactions are linear in the parameters (they are just a parameter multiplied by a new variable X1 * X2, added to other parameters) even if they are not linear in the curves 
...
Pro: CAT, item and people are interpreted on the same probability metric, some scores can be measured with more precision than others (rather than one reliability assessment), creation of item banks, investigation of differential item functioning (DIF), linkage between seperate tests

Con: complex, CTT and IRT assessments correlate >.96, 

---------

(1) the drift rate that describes a person’s tendency towards one or the other two alternatives 
(2) the boundary separation that describes how much evidence needs to be accumulated until a decision is made 
(3) the non-decision times that describes the time spent at processing the items and executing a motor response (i.e., everything non-decision related) 
(4) the initial bias that describes persons tendency towards one of the two alternatives independent of the item properties
...
-Standard  models are simple to understand (lines!), but fail to capture complex associations. There are downsides to polynomial models, as they end up making bad predictions at the ends

- We can do localized regression such as lowess but those are non-parameteric and thus we cannot do standard inferential tests
